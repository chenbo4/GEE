...
n=1000
dim=2
cor.index=cor2zcor(n,dim,2,corstr="exchangeable",otustr="exchangeable")[[1]]-1
M=dim(cor.index)[1]
K=max(cor.index)
J=1000
...
jp=list(ObtainMultBinaryDist(corr=cor.number.B, marg.probs=rep(p1,M)),ObtainMultBinaryDist(corr=cor.number.B, marg.probs=rep(p2,M)))
beta.N=beta.N.all[t]
...
for(i in 1:n){
cor.matrix=cor2zcor(n,dim,2,corstr="exchangeable",otustr="exchangeable")[[1]]-1
for(m in 1:M){
if(is.na(Z[(i-1)*M+m])){
cor.matrix[m,]=-2
cor.matrix[,m]=-2
...
zcor.reduced=rbind(zcor.reduced,zcor[(i-1)*length(temp)+1:length(temp),][temp!=-2,])
}
else if(n.cor==1){
zcor.reduced=c(as.numeric(zcor.reduced),zcor[(i-1)*length(temp)+1:length(temp)][temp!=-2])
}
}

fit1=geeglm(Y~X,family="binomial",id=id,corstr="userdefined",zcor=zcor)
fit2=geeglm(Z~X,family="gaussian",id=id,corstr="userdefined",zcor=zcor.reduced)
...
cauchy.test=0.5*tan((0.5-p.value[j,1])*pi)+0.5*tan((0.5-p.value[j,2])*pi)
...
fit4.N=lm(Z~X)
cauchy.test=0.5*tan((0.5-summary(fit4.B)$coefficients[2,4])*pi)+0.5*tan((0.5-summary(fit4.N)$coefficients[2,4])*pi)
...
fit5=geeglm(W~X,family="gaussian",id=id,corstr="userdefined",zcor=zcor)
...
for(i in 1:mod){
quantiles.B[3*(s-1)+t,2*(i-1)+1:2]=quantile(beta.est.B[,i],c(0.025,0.975),na.rm=TRUE)
quantiles.N[3*(s-1)+t,2*(i-1)+1:2]=quantile(beta.est.N[,i],c(0.025,0.975),na.rm=TRUE)
}
