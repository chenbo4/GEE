...
library(Matrix)
cor2zcor=function(n,dim,par,corstr="exchangeable",otustr="exchangeable"){
...
for(j in 1:J){
index=0
while(sum(dim[[k+1]][sum(count[[k]][1:j]):(sum(count[[k]][1:j])+index)])<=dim[[k]][j]){
index=index+1
if(sum(dim[[k+1]][sum(count[[k]][1:j]):(sum(count[[k]][1:j])+index-1)])==dim[[k]][j])
break
}
count[[k]][j]=index
}
...
squares.new=list(matrix(index.level[k]+index.taxa,nrow=dim[[k]][j],ncol=dim[[k]][j]))
...
for(t in 1:par[l]){
b=dim(block)[1]
full[b*(s-1)+1:b,b*(t-1)+1:b]=max(block)*(structure[s,t]-1)+block
if(corstr=="independence"&s!=t){
full[b*(s-1)+1:b,b*(t-1)+1:b]=matrix(0,nrow=b,ncol=b)
}
}
}
block=full
...
for(s in 1:N){
for(t in 1:N){
cor.matrix[M*(s-1)+1:M,M*(t-1)+1:M]=max(full)*(structure.OTU[s,t]-1)+full
if(s!=t){
if(otustr=="independence"){
cor.matrix[M*(s-1)+1:M,M*(t-1)+1:M]=matrix(0,nrow=M,ncol=M)
}
else if(corstr=="independence"){
cor.matrix[M*(s-1)+1:M,M*(t-1)+1:M]=structure.OTU[s,t]*full
}
}
}
}
...
zcor=matrix(nrow=n*choose(N*M,2),ncol=max(cor.matrix)-1)
